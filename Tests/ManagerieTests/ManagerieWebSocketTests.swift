...
final class ManagerieWebSocketTests: XCTestCase {
    func testHandshakeBuildUpgradeResponseSuccess() throws {
        ...
        let result = ManagerieWebSocketHandshake.buildUpgradeResponse(from: rawHeaders)
    ...
    func testFrameRoundTripTextPayload() throws {
        ...
        var buffer = ManagerieWebSocketCodec.encodeFrame(opcode: 0x1, payload: payload)

        let frame = try XCTUnwrap(ManagerieWebSocketCodec.readFrame(from: &buffer))
    ...
    func testFrameDecodeMaskedClientPayload() throws {
        ...
        var buffer = frame
        let decoded = try XCTUnwrap(ManagerieWebSocketCodec.readFrame(from: &buffer))
    ...
    func testFrameDecodeRejectsOversizedPayload() {
        ...
        let frame = ManagerieWebSocketCodec.readFrame(from: &buffer, maxPayloadBytes: 100)
        XCTAssertNil(frame)
        XCTAssertTrue(buffer.isEmpty)
    }
}
