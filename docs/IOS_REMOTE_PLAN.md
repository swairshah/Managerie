# Managerie iOS Remote App Plan

Branch: `feature/managerie-ios-remote`

## Goal

Build a removable iPhone companion app that can:

1. Discover/connect to a Managerie host on Tailscale.
...
- `apps/managerie-ios/` — iOS app only
- `Sources/Managerie/Remote/` — macOS-hosted remote transport layer only
- `docs/REMOTE_WS_PROTOCOL.md` — protocol spec shared by both sides

Minimal touch points in existing code:

- `AppDelegate` starts/stops `ManagerieRemoteServer`.
...
- [x] Add `Sources/Managerie/Remote/` module scaffold.
- [x] Add `apps/managerie-ios/` scaffold + architecture notes.

## M1 — macOS server baseline

- [x] `ManagerieRemoteServer` WebSocket listener (default `127.0.0.1:18092` + optional tailnet bind).
