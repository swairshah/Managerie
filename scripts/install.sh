...
APP_PATH="$PROJECT_ROOT/.build/Managerie.app"
EXTENSION_SRC="$PROJECT_ROOT/Extensions/pi"
CLI_SRC="$PROJECT_ROOT/.build/release/mnote"
...
# Step 4: Install Managerie.app
echo ""
echo -e "${CYAN}[4/5]${NC} Installing Managerie.app..."

if [ -d "$APP_PATH" ]; then
    INSTALL_DIR="/Applications"
    INSTALLED_APP="$INSTALL_DIR/Managerie.app"
    ...
    # Start the app
    echo ""
    echo -e "${CYAN}Starting Managerie...${NC}"
    open "$INSTALLED_APP"
else
...
# Step 5: Install mnote CLI
echo ""
echo -e "${CYAN}[5/5]${NC} Installing mnote CLI..."

if [ -f "$CLI_SRC" ]; then
    # Try /usr/local/bin first
    if [ -w "/usr/local/bin" ] || mkdir -p /usr/local/bin 2>/dev/null; then
        cp "$CLI_SRC" /usr/local/bin/mnote
        chmod +x /usr/local/bin/mnote
        echo -e "  ${GREEN}✓${NC} CLI installed to /usr/local/bin/mnote"
    else
        # Fall back to ~/.local/bin
        mkdir -p "$HOME/.local/bin"
        cp "$CLI_SRC" "$HOME/.local/bin/mnote"
        chmod +x "$HOME/.local/bin/mnote"
        echo -e "  ${GREEN}✓${NC} CLI installed to ~/.local/bin/mnote"
...
echo -e "${GREEN}==============================${NC}"
echo ""
echo "Managerie should now be running in your menu bar."
...
echo ""
echo "CLI usage:"
echo "  mnote \"Hello world\"        # Speak text"
echo "  mnote -v alba \"Hello\"      # Different voice"
echo "  echo \"Hello\" | mnote       # Pipe text"
echo ""
