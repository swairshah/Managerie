...
# Publish @swairshah/managerie from this repo.
#
# Usage:
#   scripts/publish-managerie.sh --dry-run
#   scripts/publish-managerie.sh --bump patch
#   scripts/publish-managerie.sh --version 1.2.3
#   scripts/publish-managerie.sh --bump minor --no-check-clean

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
EXT_DIR="$REPO_ROOT/Extensions/managerie"
...
usage() {
  cat <<'EOF'
Publish @swairshah/managerie from this repository.
...
echo "Tip: commit Extensions/managerie/package.json (version bump) if changed."
