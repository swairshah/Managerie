...
final class AppStore: ObservableObject {
    ...
    init() {
        ...
        socket.$snapshot
            ...
            .sink { [weak self] snapshot in
                ...
                let serverName = self.activeProfile?.displayName ?? "Managerie"
    ...
    private static let profilesKey = "managerie.profiles"
    private static let activeProfileKey = "managerie.activeProfileId"
    ...
    private func migrateFromLegacySettings() {
        let defaults = UserDefaults.standard
        let legacyHost = defaults.string(forKey: "managerie.remote.host") ?? ""
        guard !legacyHost.isEmpty, profiles.isEmpty else { return }

        let legacyPort = defaults.integer(forKey: "managerie.remote.port")
        let legacyToken = defaults.string(forKey: "managerie.remote.token") ?? ""

        let profile = ServerProfile(
            name: "My Mac",
            host: legacyHost,
            port: legacyPort > 0 ? String(legacyPort) : "18092",
            token: legacyToken
        )
        profiles.append(profile)
        activeProfileId = profile.id
        ...
        defaults.removeObject(forKey: "managerie.remote.host")
        defaults.removeObject(forKey: "managerie.remote.port")
        defaults.removeObject(forKey: "managerie.remote.token")
    }
}
