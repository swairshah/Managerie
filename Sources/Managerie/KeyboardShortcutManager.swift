...
final class KeyboardShortcutManager: ObservableObject {
    ...
    func setBinding(_ binding: ShortcutBinding?, for action: ShortcutAction) {
        ...
        debugLog("Managerie: Shortcut for \(action.displayName) set to \(binding?.displayString ?? "none")")
    }
    ...
    func resetAllToDefaults() {
        ...
        debugLog("Managerie: All shortcuts reset to defaults")
    }
    ...
    private func registerHotKey(for action: ShortcutAction, binding: ShortcutBinding) {
        guard eventHandlerRef != nil else {
            debugLog("Managerie: Cannot register hotkey - event handler not installed")
            return
        }
        ...
        if status == noErr, let ref = ref {
            ...
            debugLog("Managerie: Registered hotkey for \(action.displayName): \(binding.displayString)")
        } else {
            debugLog("Managerie: Failed to register hotkey for \(action.displayName), status=\(status)")
        }
    }
    ...
    private func unregisterHotKey(for action: ShortcutAction) {
        if let ref = hotKeyRefs.removeValue(forKey: action) {
            ...
            debugLog("Managerie: Unregistered hotkey for \(action.displayName)")
        }
    }
}
