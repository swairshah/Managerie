...
final class LocalTTSRuntime {
    ...
    // Model asset hosted alongside Managerie DMG on GitHub Releases
    private let releaseOwner = "swairshah"
    private let releaseRepo = "Managerie"
    ...
    enum RuntimeError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .binaryNotFound:
                return "Local runtime binary not found. Reinstall Managerie or install pocket-tts-cli."
            case .modelNotInstalled:
    ...
    private func downloadModelsFromReleaseAsset() async throws {
        ...
        let stagingDir = fm.temporaryDirectory.appendingPathComponent("managerie-model-stage-\(UUID().uuidString)")
    ...
    private func modelAssetURL() throws -> URL {
        if let override = ProcessInfo.processInfo.environment["MANAGERIE_LOCAL_MODEL_URL"],
        ...
        let urlString = "https://github.com/\(releaseOwner)/\(releaseRepo)/releases/download/v\(version)/Managerie-models-\(version).zip"
    ...
    private func modelCacheDirectory() -> URL {
        let base = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
            .appendingPathComponent("Managerie")
